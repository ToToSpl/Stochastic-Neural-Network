...
class Stochastic(keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        b_init = tf.random_uniform_initializer(minval=-0.1, maxval=0.1)
        self.b = tf.Variable(
            initial_value=b_init(shape=(self.units,), dtype="float32"), trainable=True
        )

    def call(self, inputs):
        sig_out = tf.math.sigmoid(tf.matmul(inputs, self.w) + self.b)
        random_vals = tf.random.uniform(
            shape=(self.units,), minval=0.0, maxval=1.0)
        greater = tf.cast(tf.greater(sig_out, random_vals), dtype=tf.float32)
